...
float createCircleTest(float O[3], float radius, float a0, float dt, float** accelPath_data, size_t* sizeAccelPath, float** circlePath_data, size_t* sizeCirclePath, float** decelPath_data, size_t* sizeDecelPath) {
	...
	float min_omega = fmin(2 * M_PI, target_omega);
...
float createHorizontalCircleTest(float O[3], float radius, float a0, float dt, int numPoints, float** circlePath_data, size_t* sizeCirclePathBuffer, int numCircles = 3) {
	//0. Compute closest omega that we can actually use, for a cyclic path (avoid discontinuities in last step in path)
	float target_omega = sqrtf(a0 / radius);
	int updatesPerCircle = (int)(2 * M_PI / (target_omega * dt) + 0.5f);
	float closest_omega = (float)(2 * M_PI / (updatesPerCircle * dt));
	...
	for (int i = 0; i < numPoints; i++)
		rotMats.push_back(glm::rotate(glm::mat4(1.f), (float)(2.f * M_PI * i / numPoints), glm::vec3(0, 0, 1)));

	//4. Copy all generated data to output buffers:
	int numSamplesTotal = numCircles * circlePositions.size() + acceleratingPositions.size() + deceleratingPositions.size();
	while (numSamplesTotal % 32 != 0) { numSamplesTotal++; }
	*sizeCirclePathBuffer = numPoints * numSamplesTotal * 4;
	*circlePath_data = new float[*sizeCirclePathBuffer];
	int totalOffset = 4 * numPoints * 0;
	for (int j = 0; j < acceleratingPositions.size(); j++) {
		glm::vec4 point = glm::vec4(acceleratingPositions[j], 1);
		int offset = totalOffset + 4 * numPoints * j;
		for (int i = 0; i < numPoints; i++) {
			glm::vec4 rotPoint = rotMats[i] * point;
			(*circlePath_data)[offset + 4 * i + 0] = rotPoint.x;
			(*circlePath_data)[offset + 4 * i + 1] = rotPoint.y;
			(*circlePath_data)[offset + 4 * i + 2] = rotPoint.z;
			(*circlePath_data)[offset + 4 * i + 3] = 1;
		}
	}
	totalOffset = 4 * numPoints * acceleratingPositions.size();
	for (int j = 0; j < numCircles * circlePositions.size(); j++) {
		glm::vec4 point = glm::vec4(circlePositions[j % circlePositions.size()], 1);
		int offset = totalOffset + 4 * numPoints * j;
		for (int i = 0; i < numPoints; i++) {
			glm::vec4 rotPoint = rotMats[i] * point;
			(*circlePath_data)[offset + 4 * i + 0] = rotPoint.x;
	...
	totalOffset = 4 * numPoints * (acceleratingPositions.size() + numCircles * circlePositions.size());
	for (int j = 0; j < deceleratingPositions.size(); j++) {
		glm::vec4 point = glm::vec4(deceleratingPositions[j], 1);
		int offset = totalOffset + 4 * numPoints * j;
		for (int i = 0; i < numPoints; i++) {
			glm::vec4 rotPoint = rotMats[i] * point;
			(*circlePath_data)[offset + 4 * i + 0] = rotPoint.x;
	...
	// Just make sure that the total number of samples is multiple of 32
	int numRests = numSamplesTotal - (numCircles * circlePositions.size() + acceleratingPositions.size() + deceleratingPositions.size());
	totalOffset = 4 * numPoints * (acceleratingPositions.size() + numCircles * circlePositions.size() + deceleratingPositions.size());
	for (int j = 0; j < numRests; j++) {
		int offset = totalOffset + 4 * numPoints * j;
		for (int i = 0; i < numPoints; i++) {
			(*circlePath_data)[offset + 4 * i + 0] = (*circlePath_data)[4 * i + 0];
			(*circlePath_data)[offset + 4 * i + 1] = (*circlePath_data)[4 * i + 1];
			(*circlePath_data)[offset + 4 * i + 2] = (*circlePath_data)[4 * i + 2];
			(*circlePath_data)[offset + 4 * i + 3] = 1;
		}
	}
	return closest_omega;
}
float createVerticalCircleTest(float O[3], float radius, float a0, float dt, int numPoints, float** circlePath_data, size_t* sizeCirclePathBuffer, int numCircles = 3) {
	//0. Compute closest omega that we can actually use, for a cyclic path (avoid discontinuities in last step in path)
	float target_omega = sqrtf(a0 / radius);
	int updatesPerCircle = (int)(2 * M_PI / (target_omega * dt) + 0.5f);
	float closest_omega = (float)(2 * M_PI / (updatesPerCircle * dt));
	float closest_v = closest_omega * radius;
	//1. Compute circle using closest_omega:
	float startPoint[] = { O[0] + radius, O[1], 0 };
	std::vector<glm::vec3> circlePositions;
	for (int s = 0; s < updatesPerCircle; s++) {
		float angle = (float)((2 * M_PI * s) / updatesPerCircle);
		glm::vec3 curPoint(cosf(angle) * startPoint[0] - sinf(angle) * startPoint[1]
	...
	std::vector<glm::mat4> rotMats;
	for (int i = 0; i < numPoints; i++)
		rotMats.push_back(glm::rotate(glm::mat4(1.f), glm::radians(90.f), glm::vec3(0, 1, 0)) * glm::rotate(glm::mat4(1.f), (float)(2.f * M_PI * i / numPoints), glm::vec3(0, 0, 1)));
	...
	*circlePath_data = new float[*sizeCirclePathBuffer];
	...
	for (int j = 0; j < acceleratingPositions.size(); j++) {
		...
		for (int i = 0; i < numPoints; i++) {
			...
			(*circlePath_data)[offset + 4 * i + 0] = rotPoint.x;
			(*circlePath_data)[offset + 4 * i + 1] = rotPoint.y;
			(*circlePath_data)[offset + 4 * i + 2] = rotPoint.z + O[2];
			(*circlePath_data)[offset + 4 * i + 3] = 1;
		}
	}
	totalOffset = 4 * numPoints * acceleratingPositions.size();
	...
	for (int j = 0; j < numRests; j++) {
		int offset = totalOffset + 4 * numPoints * j;
		for (int i = 0; i < numPoints; i++) {
			glm::vec3 tmp((*circlePath_data)[4 * i + 0], (*circlePath_data)[4 * i + 1], (*circlePath_data)[4 * i + 2]);
			(*circlePath_data)[offset + 4 * i + 0] = (*circlePath_data)[4 * i + 0];
			(*circlePath_data)[offset + 4 * i + 1] = (*circlePath_data)[4 * i + 1];
			(*circlePath_data)[offset + 4 * i + 2] = (*circlePath_data)[4 * i + 2];
			(*circlePath_data)[offset + 4 * i + 3] = 1;
...
float createArbitraryAngleCircleTest(float O[3], float radius, float a0, float dt, int numPoints, float angleDegree, float** circlePath_data, size_t* sizeCirclePathBuffer, int numCircles = 3) {
	//0. Compute closest omega that we can actually use, for a cyclic path (avoid discontinuities in last step in path)
	float target_omega = sqrtf(a0 / radius);
	int updatesPerCircle = (int)(2 * M_PI / (target_omega * dt) + 0.5f);
	float closest_omega = (float)(2 * M_PI / (updatesPerCircle * dt));
	...
	std::vector<glm::mat4> rotMats;
	for (int i = 0; i < numPoints; i++)
		rotMats.push_back(glm::rotate(glm::mat4(1.f), glm::radians(angleDegree), glm::vec3(0, 1, 0)) * glm::rotate(glm::mat4(1.f), (float)(2.f * M_PI * i / numPoints), glm::vec3(0, 0, 1)));
