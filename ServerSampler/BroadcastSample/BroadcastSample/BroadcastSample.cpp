...
{
    printf("Weicheng Acoustic Levitation MSc Project Broadcast Server v240809\n\n");

    // BroadcastSample Original
    int retval;
    WSADATA WsaData;
    int value = 1;
    ...
    //if (argc > 2)
    //{
    strcpy_s(szReceiveAddress, "127.0.0.1");	// specified on command line
    ...
    printf("\nCommands from writing a file:\n0 : Send XML command to Motive to start recording.\n1 : Send XML command to Motive to stop recording.\n9 : exit");

    printf("\n\nListening...\n");

    // System File Checker Loop
    std::filesystem::path filePath("C:/Users/weicheng/Desktop/ServerSampler/bin/example.txt");
    bool running = false;
    int mode = 0;
    ...
    while (true) {
        ...
        if (std::filesystem::exists(filePath)) {
            ...
            if (file.is_open()) {
                ...
                while (std::getline(file, line) && lines.size() < 3) {
                    lines.push_back(line);
                }

                file.close();  // Close the file before attempting to delete it

                if (lines.size() == 3 && isNumber(lines[0])) {
                    int number = std::stoi(lines[0]);  // Convert the first line to a number
                    name = lines[1];       // Second line is the name
                    folderPath = lines[2]; // Third line is the folder path
                    ...
                    //std::cerr << "File does not contain exactly three lines or the first line is not a valid number. File not deleted." << std::endl;
                }
        ...
        if (running) {
            ...
            if (mode == 0) {
                ...
                sprintf(szMessage, "<CaptureStart><TimeCode VALUE=\"00:00:00:00\"/><Name VALUE=\"%s\"/><Notes VALUE=\"\"/><Assets VALUE=\"\"/><SessionName VALUE=\"Session1\"/><Description VALUE=\"\"/><DatabasePath VALUE=\"%s\"/><PacketID VALUE=\"0\"/><ProcessID VALUE=\"%d\"/></CaptureStart>",
                    name.c_str(), folderPath.c_str(), pid);
                ...
                sprintf(szMessage, "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?><CaptureStop><TimeCode VALUE=\"00:00:00:00\"/><Assets VALUE=\"\"/><Name VALUE=\"%s\"/><Notes VALUE=\"\"/><Description VALUE=\"\"/><DatabasePath VALUE=\"%s\"/><PacketID VALUE=\"0\"/><ProcessID VALUE=\"%d\"/></CaptureStop>", name.c_str(), folderPath.c_str(), pid);
                retval = sendto(receiverSocket, (char*)szMessage, 4 + strlen(szMessage), 0, (sockaddr*)&ToAddr, sizeof(ToAddr));
                if (retval != SOCKET_ERROR)
                {
                    printf("\nSENT (to %d.%d.%d.%d:%d) : %s\n",
